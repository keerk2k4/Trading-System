...
public interface OrderMapper {
     ...
     */
    @Insert("""
        INSERT INTO orders (trading_account_id, instrument_id, order_type, side, product_type, quantity, limit_price, stop_price, status, idempotency_key, created_at, updated_at)
        VALUES (#{order.account.accountId}, #{order.instrument.instrumentId}, #{order.orderType}, #{order.side}, #{order.productType}, #{order.quantity}, #{order.limitPrice}, NULL, 'NEW', #{order.idempotencykey}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
